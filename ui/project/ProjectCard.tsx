...
export default function ProjectCard({
    ...
    return <>{(usePromise) && <Link className="w-100 select-none transition hover:scale-103 cursor-pointer group h-max" href={link} target="_blank">
        <ImageSkeleton width={1600} height={900} alt={`An Image of ${title}`} draggable={false} className="w-full h-30 rounded-t-4xl object-cover transition group-hover:grayscale-90" promise={usePromise}/>
