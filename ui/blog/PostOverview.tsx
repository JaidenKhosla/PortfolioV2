...
export default async function PostOverview({ id, created_at, title, description, author, tags } : BlogInfo){
    return <Link href={`/blog/${title}`} prefetch={true}>
                ...
                <ImageSkeleton promise={serveFile("portfolio-images", title)} alt={title} width={1600} height={900} className="w-[24rem] h-[13.5rem] rounded-2xl transition-all group-hover:scale-105"/>
